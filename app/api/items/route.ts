...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!session || !session.user) {
      return NextResponse.json({ error: "Unauthorized: Please login first (Missing CSRF or invalid Session Cookie)" }, { status: 401 });
    }

    if (role !== "admin" && role !== "uploader") {
      return NextResponse.json({ error: `Forbidden: Restricted Role (${role})` }, { status: 403 });
    }
    ...
    const qrDataUrl = await qrcode.toDataURL(id, { width: 300, errorCorrectionLevel: 'H' });
    ...
    return NextResponse.json({ success: true, item: { ...inserted[0], qrCodeUrl } }, { status: 201 });
  } catch (error: any) {
